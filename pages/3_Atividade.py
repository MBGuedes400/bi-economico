...
visao = "Oferta (setores)"

def _filtros():
    global ano_ini, ano_fim, visao
...
if not df_ibcbr.empty:
    ...
    df_ibcbr_f = df_ibcbr_r[(df_ibcbr_r.index >= di) & (df_ibcbr_r.index <= df_fim)]
else:
...
if not df_pib_f.empty:
    ...
    if len(df_tot) >= 5:
        pib_var_anual = round((df_tot.iloc[-1]["Valor"] / df_tot.iloc[-5]["Valor"] - 1) * 100, 2)
...
pib_focus = None
if not df_fa.empty:
    f = df_fa[(df_fa["Indicador"] == "PIB Total") & (df_fa["DataReferencia"] == str(ano_at))]
    if not f.empty:
        pib_focus = round(float(f.sort_values("Data").iloc[-1]["Mediana"]), 2)

setor_destaque = setor_val = None
crescimentos = {}
if not df_pib_f.empty and pib_var_anual is not None:
    for setor in ["Agropecuaria", "Industria", "Servicos"]:
        df_s = df_pib_f[df_pib_f["Setor"] == setor].sort_values("Data")
        if len(df_s) >= 5:
            crescimentos[setor] = round((df_s.iloc[-1]["Valor"] / df_s.iloc[-5]["Valor"] - 1) * 100, 2)
    if crescimentos:
...
with c2:
    st.metric("Var. Anual PIB", f"{pib_var_anual:+.2f}%" if pib_var_anual else "—",
              delta="vs mesmo tri ano anterior",
              delta_color="normal" if pib_var_anual and pib_var_anual > 0 else "inverse")
with c3:
...
with col_g1:
    if visao == "Oferta (setores)":
        ...
        CORES_SETORES = {"Agropecuaria": "#00D4AA", "Industria": "#FFB800", "Servicos": "#00D4FF"}
    else:
        st.markdown("#### PIB por Componente de Demanda — R$ bilhões")
        SETORES_PLOT  = ["Consumo_Familias", "Consumo_Governo", "Investimento_FBCF", "Exportacoes"]
        CORES_SETORES = {
            "Consumo_Familias": "#00D4FF", "Consumo_Governo": "#FFB800",
            "Investimento_FBCF": "#00D4AA", "Exportacoes": "#FF6B6B",
        }
    ...
    if not df_pib_f.empty:
        ...
        for setor in SETORES_PLOT:
            df_s = df_pib_f[df_pib_f["Setor"] == setor].sort_values("Data")
            if not df_s.empty:
                ax.plot(df_s["Data"], df_s["Valor"] / 1000,
        ...
        ax.yaxis.set_major_formatter(mticker.FuncFormatter(lambda x, _: f"R$ {x:.0f} bi"))
...
with col_g2:
    ...
    if not df_ibcbr_f.empty and "IBC_Br" in df_ibcbr_f.columns:
        ...
        ax2.plot(s.index, s.values, color="#00D4FF", lw=2.0, alpha=0.9)
        ax2.plot(mm3.index, mm3.values, color="#FFB800", lw=2.0, ls="--", label="Média móvel 3m")
        ax2.grid(True, color="#FFF", alpha=0.05, lw=0.5)
        ax2.tick_params(colors="#AAAAAA", labelsize=8)
        ax2.xaxis.set_major_formatter(mdates.DateFormatter("%b/%y"))
        plt.xticks(rotation=30, ha="right")
        for sp in ax2.spines.values(): sp.set_edgecolor("#333")
        ax2.set_title("IBC-Br (índice)", color="white", fontsize=11, fontweight="bold", pad=12)
...
with col_t:
    ...
    if pib_var_anual is not None and pib_total_dt:
        ritmo = ("acima da média histórica" if pib_var_anual > 3
                 else "em ritmo moderado" if pib_var_anual > 1
                 else "em ritmo fraco" if pib_var_anual > 0
                 else "em contração")
        linhas.append(f"**[PIB]** No {pib_total_dt}, o PIB cresceu **{pib_var_anual:+.2f}%** "
    ...
    if setor_destaque and crescimentos:
        ...
        linhas.append(f"**[Setores]** O setor de **{setor_destaque}** liderou o crescimento "
                      f"com **+{setor_val:.1f}%** a/a. " +
                      " · ".join([f"{k}: {v:+.1f}%" for k, v in outros.items()]))
    if pib_focus:
        linhas.append(f"**[Expectativa]** O mercado (Focus) espera crescimento de "
                      f"**{pib_focus:.2f}%** para o PIB em {ano_at}.")
    if ibcbr_v:
        linhas.append(f"**[IBC-Br]** O Índice de Atividade do BCB registrou **{ibcbr_v:.2f}** "
...
with col_e:
    ...
    if not df_fa.empty:
        ...
        if not df_pib_fa.empty:
            anos_ref = [a for a in sorted(df_pib_fa["DataReferencia"].unique())
                        if int(a) >= ano_at][:3]
            for ar in anos_ref:
