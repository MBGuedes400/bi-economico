...
moedas_sel = ["USD_BRL", "EUR_BRL", "CNY_BRL"]

def _filtros():
    global ano_ini, ano_fim, moedas_sel
...
with c1:
    st.metric("USD / BRL", f"R$ {v_usd:.4f}" if v_usd else "—",
              delta=f"{var_usd:+.2f}%" if var_usd is not None else "último período",
              delta_color="inverse" if var_usd and var_usd > 0 else "normal")
with c2:
    st.metric("EUR / BRL", f"R$ {v_eur:.4f}" if v_eur else "—",
              delta="Euro", delta_color="off")
with c3:
    st.metric("CNY / BRL", f"R$ {v_cny:.4f}" if v_cny else "—",
              delta="Yuan chinês", delta_color="off")
with c4:
    st.metric("Reservas Internacionais", f"US$ {v_res:.1f} bi" if v_res else "—",
              delta=f"{var_res:+.2f}%" if var_res is not None else "último período",
              delta_color="normal" if var_res and var_res > 0 else "inverse")
with c5:
    st.metric(f"Focus Câmbio {ano_at}", f"R$ {fc_at:.2f}" if fc_at else "—",
...
# -----------------------------------------------------------------------------
CORES_MOEDA = {
    "USD_BRL": "#00D4FF", "EUR_BRL": "#FFB800",
    "CNY_BRL": "#00D4AA", "GBP_BRL": "#A78BFA",
}
EVENTOS = [
...
with col_g1:
    ...
    if df_c.empty or not moedas_sel:
        ...
        for moeda in moedas_sel:
            if moeda not in df_c.columns: continue
            s = df_c[moeda].dropna()
            if s.empty: continue
            ax.plot(s.index, s.values, color=CORES_MOEDA.get(moeda, "#AAAAAA"),
                    lw=2.0, label=moedas_disp[moeda], marker="o", ms=2)
        ymin, ymax = ax.get_ylim()
        for ev in EVENTOS:
            ev_dt = pd.Timestamp(ev["data"])
            if ev_dt < di or ev_dt > df_fim: continue
            ax.axvline(ev_dt, color=ev["cor"], lw=0.8, ls="--", alpha=0.6)
            ax.text(ev_dt, ymax, ev["label"], color=ev["cor"], fontsize=6.5,
...
with col_g2:
    ...
    if df_c.empty or "Reservas_USD_bi" not in df_c.columns:
        ...
        ax2.fill_between(s_res.index, s_res.values, alpha=0.15, color="#00D4AA")
        ax2.plot(s_res.index, s_res.values, color="#00D4AA", lw=2.0, alpha=0.9, label="Reservas")
        mm12 = s_res.rolling(12, min_periods=3).mean()
        ax2.plot(mm12.index, mm12.values, color="#FFB800", lw=1.8, ls="--", label="Média 12m")
        ...
        ax2.set_title("Reservas (US$ bi)", color="white", fontsize=11, fontweight="bold", pad=12)
...
with col_t:
    ...
    if v_usd and d_usd:
        ...
        txt = (f"**[Câmbio]** O dólar encerrou **{d_usd.strftime('%B/%Y')}** "
               f"cotado a **{usd_fmt}**")
    ...
    if fc_at or fc_prx:
        ...
        linhas.append(f"**[Expectativa]** O mercado (Focus/BCB) projeta câmbio em "
                      f"**{fc_s}** para {ano_at} e **{fc_s2}** para {ano_at + 1}.")
    linhas.append("**[Contexto]** O câmbio brasileiro é influenciado pelo diferencial de juros "
